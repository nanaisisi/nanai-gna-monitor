...
このリポジトリでは実行ファイル名が `nanai-gna-monitor.exe` です。
...
4. パッケージ化して署名します：
   ...
   ```powershell
   winapp package .\dist --manifest .\dist\Package.appxmanifest --cert .\devcert.pfx --output nanai-gna-monitor.msix
   ```
